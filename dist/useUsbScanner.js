...
function useUsbScanner(options) {
    ...
    const [savedPortInfo, setSavedPortInfo] = useState(typeof window !== "undefined"
    ...
    const connectToPort = async () => {
        ...
        if (savedPortInfo) {
            ...
            selectedPort = allPorts.find((port) => {
                const info = port.getInfo();
                return (info.usbVendorId === previousPortInfo.usbVendorId &&
    ...
    const readDataFromPort = async (port) => {
        ...
        try {
            while (true) {
                ...
                lines.forEach((line) => {
                    ...
                    setParsedData(options?.parser ? options.parser(line) : { rawData: line } // Default parser just returns the raw data
